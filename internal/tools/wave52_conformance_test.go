...
// Wave 52 / POL-005 + MCP-001 + NET-003 + MUT-001 + DIAG conformance (Track D):
...
// TestWave52_Wave50Done_OperatorCapsAbsConcurrentBackoff_Hard hard-asserts
...
// retention. Must remain true after Wave 52 parallel tracks merge.
func TestWave52_Wave50Done_OperatorCapsAbsConcurrentBackoff_Hard(t *testing.T) {
	t.Parallel()
...
// TestWave52_Wave51Done_TargetBytesResolve_Hard hard-asserts Wave 51 Track C Done*:
...
func TestWave52_Wave51Done_TargetBytesResolve_Hard(t *testing.T) {
	t.Parallel()
...
// TestWave52_Wave51Done_SurveyDiagnoseCeilings_Hard hard-asserts Wave 51 Track B
// Done*: operator_caps survey/diagnose package hard ceiling keys are present
// offline and positive.
func TestWave52_Wave51Done_SurveyDiagnoseCeilings_Hard(t *testing.T) {
	t.Parallel()
	...
	if caps == nil || caps.Details == nil {
		t.Fatal("operator_caps_snapshot missing (Wave 51 Track B requires offline snapshot)")
	}
...
func TestWave52_SoftResidual_TrackB_MinAbsBackoffMsKeys(t *testing.T) {
	...
	// Progressive keys Track B may add once min/absolute backoff honesty lands
	// (Wave 52 Track B). Defaults (default_*_backoff_ms) are already hard above.
	progressivePositive := []string{
		...
		// Alternate naming residuals (if exposed under different keys).
