...
func TestBuildGatewayResidualStatus_SubjectLimiterMaxSubjects(t *testing.T) {
	...
	// Explicit empty getenv → omit.
	clear := diagnostics.BuildGatewayResidualStatus(func(string) string { return "" })
	if _, ok := clear["subject_limiter_max_subjects"]; ok {
		t.Fatalf("empty getenv must omit subject_limiter_max_subjects: %+v", clear["subject_limiter_max_subjects"])
	}

	t.Setenv(gateway.EnvGatewaySubjectLimiterMaxSubjects, "64")
	out = diagnostics.BuildGatewayResidualStatus(nil)
	if out["subject_limiter_max_subjects"] != 64 {
		t.Fatalf("subject_limiter_max_subjects: %+v want 64", out["subject_limiter_max_subjects"])
	}
	// Path residual fields stay false — max subjects is not a path residual.
	if out["shared_subject_rate_file"] != false {
		t.Fatalf("shared_subject_rate_file must stay false without path: %+v", out["shared_subject_rate_file"])
	}
	if out["shared_principal_cache_file"] != false {
		t.Fatalf("shared_principal_cache_file must stay false without path: %+v", out["shared_principal_cache_file"])
	}
	if out["shared_jwks_file"] != false {
		t.Fatalf("shared_jwks_file must stay false without path: %+v", out["shared_jwks_file"])
	}
	...
	// Zero is unlimited → omit (same as empty).
	t.Setenv(gateway.EnvGatewaySubjectLimiterMaxSubjects, "0")
	out = diagnostics.BuildGatewayResidualStatus(nil)
	if _, ok := out["subject_limiter_max_subjects"]; ok {
		t.Fatalf("0 must omit subject_limiter_max_subjects (unlimited): %+v", out["subject_limiter_max_subjects"])
	}
}
