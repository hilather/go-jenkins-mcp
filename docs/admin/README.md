...
| Gateway Ready / HA | Admin health always reports `gatewayReady: false` and `haMultiReplica: false` (admin BFF ≠ MCP serve `/readyz`; HOST-008 single-replica Tier A). When multi-user env is set, `sessionAffinityRecommended: true` (kustomize sticky scaffold honesty only — not multi-replica Done). Live Ready is on the gateway serve process only. |
