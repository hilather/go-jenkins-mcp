...
|---------|---------------------|
| Overview / policy effective / doctor read | Loopback operator chrome: grouped rail Status / Config / Ops; profile + admin token stay in the rail footer (localStorage Bearer, no login page). Overview is a snapshot (hero chips + runtime / live pins); residual voice is badge + one-line caveat + HOST-* details. **MCP-OPS:** same reads available as `admin_*` tools when serve uses `--enable-admin-mcp` (default off) — [mcp-ops-parity.md](mcp-ops-parity.md) |
...
| **Metrics** (UI-005) | Auto-refresh (15s) with pause on hidden tab + manual pause; **Apache ECharts** tool-outcome counts (y includes 0), one cache usage/quota meter, subject-quota lines; HTTP counts/bytes as tables; session ring ≤60; export is the current snapshot only. Preferred keys match the Go registry. **Residual:** process-local only; no fleet aggregation |
