...
optional max/ttl, optional `subject_limiter_max_subjects` when env set, always
...
and progressive consent store honesty (`file_backed` / `same_host_reload_before_persist`
when `JENKINS_MCP_CONSENT_STORE_PATH` set; `multi_replica_shared=false`;
`stores_tokens=false` — path never shown; not multi-pod HA). Operators may also
run CLI `gateway residual-status`.
