...
| **FND-007** | Done* | GitHub Actions CI: merge gate = `lint-test-build` + `govulncheck`; optional `package-smoke` (Ubuntu), `fuzz-smoke` (count-based `FUZZTIME=500x` via `scripts/fuzz-smoke.sh`), `stdio-smoke` (Wave 26, continue-on-error), perf step; live Jenkins = workflow_dispatch only. *Residuals: Cursor host CI, SBOM attach, secret/code scanning productization, required-check owner config. |
| **FLC-000 / FLC-001** | Done (docs) | Peer shared-cache Phase 0: audit `docs/fleet/shared-cache-current-state.md` (snapshot `0bde636…`), ADR 0016 Accepted, `FLC-*` reserved in task-index (runtime **Planned**); MVP A = peer read first. *Not* peer-cache runtime Done. |
