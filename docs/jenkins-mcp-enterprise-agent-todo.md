...
4. **Documentation always updated:** behavior, CLI/tool contracts, architecture, packaging, agent guidance, and root **`README.md`** (release/phase/features/quick start when landing would otherwise be stale) stay in sync with the code in the same change; no silent capability claims. See `AGENTS.md` → *Root README currency*.
