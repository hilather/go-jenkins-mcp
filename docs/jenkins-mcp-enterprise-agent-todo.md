...
- [x] Admin health/vault shared file residual parity: camelCase `sharedSubjectRateFile` / `sharedPrincipalCacheFile` / `sharedJwksFile` (true when env paths set; path values never returned; same-host lite only — **not** multi-pod HA). Mirrors residual-status snake_case. SPA Overview health + vault cards. — BFF tests + docs/admin/api-v1.md
