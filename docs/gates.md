...
| MCP stdio binary host-lifecycle smoke (opt-in) | `make stdio-smoke` | FND-006 Wave 25 + Wave 33 offline binary host-lifecycle over stdio; Wave 26 optional CI job `stdio-smoke` (continue-on-error, not merge-gate); residual id `stdio_binary_smoke` / offline host-lifecycle Done*; Cursor product binary CI residual |
...
- Cursor product binary / host stdio CI remains residual (offline MCP protocol matrix + Wave 25/33 binary host-lifecycle smoke + Wave 26 optional CI job `stdio-smoke`; not real Cursor). Residual ids: offline host-lifecycle / `stdio_binary_smoke` Done* vs `cursor_host_ci` open.
