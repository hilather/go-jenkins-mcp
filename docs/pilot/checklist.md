...
**Pilot evidence residual lite:** `make pilot-evidence` also captures `gateway-residual-status.json` (+ optional `gateway-consent-residual.json`) into the pack with residual-id / `ha_multi_replica=false` / `shared_*_file` default-false / `subject_limiter_max_subjects` omit default (+ lightweight path-not-dumped + `SUBJECT_LIMITER_MAX_SUBJECTS=64` → field 64) honesty canaries — no separate residual-smoke required for residual presence in pilot kits (fuller residual-smoke suite still optional).  
