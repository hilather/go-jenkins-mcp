...
| Multi-fleet **peer** sealed-log cache (FLC) | Library **implemented; mode default **off**; operator canary [fleet/shared-cache-operator.md](fleet/shared-cache-operator.md); audit [fleet/shared-cache-current-state.md](fleet/shared-cache-current-state.md); ADR [0016](adr/0016-fleet-p2p-shared-cache.md); offline **FLC-073 implemented; live multi-host residual (not automatic site production GO) |
