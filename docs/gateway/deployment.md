...
| 8 | Shared subject rate / concurrency limiters | Process-local `SubjectRateLimiter` / `SubjectLimiter` only today | **Residual** (admin `rateEnabled` is env residual only; external rate residual when scaling) |
...
| Admin `GET /admin/v1/health` | `multiUserEnabled`, `credentialMode`, `gatewayReady=false`, `haMultiReplica=false`, `sessionAffinityRecommended` (true when multi-user env set), `rateEnabled` | Admin BFF ≠ MCP serve; Ready residual documented; `rateEnabled` = HOST-006 env parse only (process-local) |
| Admin `GET /admin/v1/gateway/vault` | `multiUserEnabled`, `haMultiReplica=false`, `sessionAffinityRecommended`, `rateEnabled` + mode matrix | Never tokens; multi-user residual note when env set; file vault flock is multi-process lite only |
