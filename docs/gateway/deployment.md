...
| Subject limiter / rate | Concurrency slots process-local. Rate: default process-local `SubjectRateLimiter` (`shared_subject_rate_file: false`). Optional **same-host** `FileSubjectRateLimiter` via `JENKINS_MCP_GATEWAY_SUBJECT_RATE_PATH` (flock + secret-free JSON 0600; `shared_subject_rate_file: true`). Optional **MaxSubjects** map hygiene via `JENKINS_MCP_GATEWAY_SUBJECT_RATE_MAX_SUBJECTS` (empty = unlimited; LRU/oldest lastAccess; process-local / file-local only). `ha_multi_replica: false` always — multi-pod shared rate residual |
