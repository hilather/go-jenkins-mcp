...
| Provider !Ready | Residual local session (Mode C default Live=false / Mode B HOST-010 residual) |

Mode A Ready → Basic personal vault token. Mode C Ready (Live opt-in) → Bearer
access token. Mode B remains residual (`jwt_rs_bearer` not Ready).
...
multi-user gateway without a local bootstrap session is HOST-001 / mid-session
rebind residual — never a shared SA substitute.
