...
| Operator residual surfaces (`doctor` `gateway_status`, `gateway qualify` residual row, `gateway residual-status`, admin `GET /admin/v1/gateway/residual-status`, `gateway consent-residual`) | **Done\*** (env/static honesty) |
...
Shared assembly: `diagnostics.BuildGatewayResidualStatus`. Never tokens or subjects.
Env/static only — not live Ready / production GO.
