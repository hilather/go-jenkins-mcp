...
| **MUT-001** | Mutation preview/confirm; token TTL + confirm cooldown; process-local rate limit; Binding (profile+principal+ExternalSubject+tenant) | `internal/mutation` Manager (DefaultTokenTTL 2m, DefaultConfirmCooldown 5s); multi-user `BindingFromContext` | `mutation_confirm_cooldown_residual`; Alice/Bob binding tests | Offline Preview+Confirm + immediate re-Confirm denied (`confirm_cooldown`); Alice token rejected for Bob (`binding_mismatch`); mutations remain opt-in / RO default; residual per-request Jenkins principal on Binding |
