...
| **MUT-001** | Mutation preview/confirm; token TTL + confirm cooldown; process-local rate limit | `internal/mutation` Manager (DefaultTokenTTL 2m, DefaultConfirmCooldown 5s) | `mutation_confirm_cooldown_residual` | Offline Preview+Confirm + immediate re-Confirm denied (`confirm_cooldown`); mutations remain opt-in / RO default; gateway multi-tenant residual |
...
| **OAUTH-009** | jwt-auth-filter / RS offline contracts + OfflineFallthroughFixtures matrix | `rs_qualification.go`, `rs_fallthrough.go` | `rs_qualification` | Fixture count ≥12; `live_lab_still_required`; offline fallthrough/JWKS/routes; live lab residual |
...
| **MGR-001** | Signed policy bundles (Ed25519) | `bundle.go`, `ed25519.go` | `policy_signature_mode` | Production: trusted keys + RequireSigned |
