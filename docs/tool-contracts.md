...
Successful execute remains single-use (token discarded). Cooldown does not re-authorize a consumed token; issue a new preview after cooldown if needed. Audit deny events carry reason codes only (no secrets). Offline self-check canary `mutation_confirm_cooldown_residual` (MUT-001) proves default TTL/cooldown and fail-closed re-confirm deny without network; mutations remain opt-in (`--allow-mutations`).
