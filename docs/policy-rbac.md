...
| **Job path normalize (Wave 30)** | `Target.JobName` via `policy.NormalizeJobFullName` (same as MatchDenyJobPattern): collapse empty segments / leading `/` so `prod//secret` → `prod/secret`. Path traversal (`..`) fails closed to empty JobName |
