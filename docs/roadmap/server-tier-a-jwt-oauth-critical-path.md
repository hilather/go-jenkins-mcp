...
**Status:** Offline + **free-lab** Tier A **implemented (product bar — see [free-lab-qualification.md](../gateway/free-lab-qualification.md)). Mode A disposable Jenkins vault Obtain **implemented; Mode B mock RS + Mode C mock-token **implemented (`make live-oauth-test`). **Site production** Entra / jwt-auth-filter pins remain **operator-owned residual** (`mode_*_live_*_qualified=false` until **that site** attaches evidence). Free labs do **not** flip production-qualified flags.  
**Audience:** implementers, security, platform  
**Defer:** multi-node / multi-pod HA (**HOST-008** Tier B) — same-host flock lite already implemented  
...
|----|------|------|----------------|-------------------|
| **S0.1** | **HOST-001** Streamable HTTP multi-user: RequireSubject, session fingerprint, mid-session rebind fail-closed | M | implemented offline + lab JWT Alice/Bob | Live Entra JWKS under load; path-prefix pin with real edge |
| **S0.2** | **HOST-002** Reverse-proxy Host/Origin matrix; TrustedProxy honesty; dual health/ready | S–M | Offline fixtures implemented | Live edge container matrix (X-Forwarded ignore) |
...
| **S0.4** | **HOST-003** Serve Obtain wire: credential mode → Jenkins client; Ready fail-closed; Live only when explicit | M | implemented offline modes | Live Obtain for each enabled mode |
| **S0.5** | **HOST-004** Cache/archive partition by tenant/profile/subject on multi-user host | M | implemented offline (SubjectKey + token/principal/vault partition; `TestCriticalPath_HOST004_*`) | Live concurrent two-user load under real edge |
| **S0.6** | **HOST-006** Per-subject rate + concurrency (process-local + same-host file lite OK) | S | implemented process/file lite | Measure under live multi-user load |
| **S0.7** | Doctor / residual-status / residual-smoke honesty: `mode_*_live_*_qualified=false` until pins | S | implemented | Never flip live flags without evidence |
...
| ID | Task | Size | Notes |
|----|------|------|-------|
| **S1.1** | **HOST-009** Vault lifecycle: put/list/status/delete/revoke CLI; Obtain returns **only** bound subject token | M | Offline implemented; admin vault **write** residual |
| **S1.2** | Obtain isolation canaries: cross-subject read fail closed; no process-wide default token | S | implemented file vault + multi-user HTTP tools/call Alice/Bob |
| **S1.3** | Secret-free: no token in logs, admin JSON, MCP, support bundle | S | implemented canaries; keep green |
| **S1.4** | Live lab: multi-user HTTP + vault subjects against real Jenkins Basic ACL | M | implemented lab: offline multi-user HTTP+vault + `TestLive_ModeAVaultObtain_WhoAmI` (`make live-jenkins-test`, `-tags=live_jenkins`). Production multi-user load residual |
| **S1.5** | RO pilot + optional mutations still fail closed per subject | S | MUT already subject-bound offline implemented |

**Exit S1:** Offline Mode A complete. **Disposable Jenkins vault Obtain lab implemented (`make live-jenkins-test`). Production multi-user load + pin flag flip still residual.
...
| ID | Task | Size | Notes |
|----|------|------|-------|
| **S2.1** | **OAUTH-009** product free-lab + offline RS contracts; checklist for **site** pin (live-pin-blockers §2) | L | Product implemented; site LTS/plugin/JCasC fill-in = operator residual |
...
| **S2.3** | Invalid Bearer matrix: offline + mock RS implemented | M | `make live-oauth-test`; real jwt-auth-filter on site = operator residual |
| **S2.4** | JWKS outage + rotation contracts offline | M | Fail closed; site concurrent load = operator residual |
| **S2.5** | Entra group overage fail-closed (no invent membership); Graph expansion residual noted | S | OAUTH-006 foundation implemented |
| **S2.6** | **Approved reverse-proxy RS path** documented as first-class option | M | Site chooses plugin vs proxy |
| **S2.7** | **HOST-010** Gateway Mode B Bearer-only | M | Offline + mock-rs implemented |
...
| **S3.2** | Authorization-code **3LO** Obtain: consent metadata only (never tokens in SPA/audit) | L | Progressive consent implemented metadata; browser 3LO against Entra = operator residual |
| **S3.3** | **OBO / RFC 8693 / RFC 7523** exchange path shape → short-lived Jenkins-audience access token | L | Fail closed without exact aud; free mock-token lab implemented |
| **S3.4** | `HTTPTokenFetcher` Live=true only under explicit config; https-only; no redirects; body caps | M | Offline + mock-token lab implemented (`make live-oauth-test` HOST-015). Site Entra = operator residual |
| **S3.5** | Consent session store: file-backed reload-before-persist lite; purge CLI/admin; CLEAR_ALL confirm | S | Offline implemented |
| **S3.6** | Force re-auth / revoke window: subject-invalidate + principal/token cache clear (not multi-pod fan-out) | M | Offline implemented; live IdP revoke residual |
...
| **S3.9** | Never AS-to-Jenkins: config + doctor reject Jenkins as token endpoint | S | implemented offline (`ValidateProviderConfig` + critical-path canary); keep green |
...
| ID | Task | Size | Notes |
|----|------|------|-------|
| **S4.1** | Config enum: enable modes independently (`api_token_vault`, `jwt_rs_bearer`, `agentcore_3lo_obo`) | S | Offline implemented |
| **S4.2** | No silent fallthrough: failed Mode B never becomes another subject’s Mode A token | S | Offline implemented (qualify + `TestCriticalPath_*`); live re-prove |
| **S4.3** | LIVE env only legal on Mode C; reject on A/B | S | Offline implemented |
...
| ID | Task | Size | Notes |
|----|------|------|-------|
| **S5.1** | **HOST-005 / GWY-004** Deploy scaffold: compose/kustomize `replicas: 1`, probes, resource limits | S | implemented scaffold |
| **S5.2** | Sticky session optional honesty only (do not claim HA) | S | implemented |
| **S5.3** | Env completeness: modes, vault paths, JWKS, multi-user, consent path | S | Partial |
| **S5.4** | `make residual-smoke` + pilot-evidence pack still list residual ids | S | implemented offline |
...
| ID | Task | Size | Notes |
|----|------|------|-------|
| **S6.1** | **HOST-012…014** Mock IdP + mock JWT RS + route fallthrough lab | M | implemented (`make live-oauth-*` + Mode B live_oauth tests); production plugin residual |
| **S6.2** | **HOST-015** Mock token peer for Mode C Obtain shape | S | implemented scaffold + Mode C live_oauth; Entra residual |
| **S6.3** | Free: Keycloak OIDC + Jenkins + real jwt-auth-filter | L | **implemented opt-in `make live-jwt-rs-*` (`testdata/jwt-rs-lab/`); site Entra residual |
...
- [x] Mode B **product**: offline Bearer matrix + mock RS lab implemented (`make live-oauth-test`). **Site** production jwt-auth-filter/proxy pin = operator residual (checklist in live-pin-blockers / jwt-auth-filter-qualification).  
- [x] Mode C **product**: offline mock AS + mock-token lab implemented + Jenkins-as-AS reject. **Site** Entra/AgentCore pin = operator residual.  
...
| Agent backlog IDs | HOST-001…011, OAUTH-009/010, GWY-001…004 in `docs/archive/jenkins-mcp-enterprise-agent-todo.md` (archived; open work → Issues) |
