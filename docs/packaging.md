...
|-------------|--------|--------|
| Offline host-lifecycle matrix (unit + binary smoke) | **implemented | Unit protocol matrix + `make stdio-smoke` host-lifecycle expansion |
...
**Residual (not closed):** **Cursor product binary / host stdio CI** — real Cursor (or equivalent MCP host) process spawning this binary over stdio via product `mcpServers` config. Offline host-lifecycle matrix is **implemented; unit tests and binary smoke deliberately do **not** require a Cursor binary or Docker. Pilot operators still validate product stdio manually via [user guide](user/README.md).
