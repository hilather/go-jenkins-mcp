...
- **Server-side residuals closed (foundation):** HOST-003 Obtain→Jenkins AuthProvider when Ready (Mode A + Mode C Live opt-in; clear static; whoAmI via Obtain; ConsentRequired metadata; no other-subject fallthrough); HOST-001 JWKS env wire on serve (`JENKINS_MCP_HTTP_JWKS_*`); HOST-011 mode matrix + Mode B residual provider; admin `GET /admin/v1/gateway/vault` (hash-only); **GWY-001 Live opt-in foundation** (`JENKINS_MCP_GATEWAY_LIVE` + `EnableLiveHTTPFetcher` / `HTTPTokenFetcher` — **not** full GWY-001 Done). Residual: mid-session subject rebind, live Entra/jwt-auth-filter plugin, real AgentCore pin, GWY-003 live qualify.
