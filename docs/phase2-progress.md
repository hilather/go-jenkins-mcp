...
- **Server-side residuals closed (foundation):** HOST-003 Obtain→Jenkins when Ready (A/B vault + C Live opt-in; whoAmI via Obtain); HOST-001 JWKS env + mid-session `Mcp-Session-Id` fingerprint fail-closed offline; HOST-010 Mode B JWT vault; HOST-011 matrix; GWY-002 claim helpers; GWY-001 Live opt-in foundation; admin vault hash-only. Residual: continuous JWKS rotation under load, **live Entra/jwt-auth-filter (OAUTH-009)**, real AgentCore pin, GWY-003 live qualify.
