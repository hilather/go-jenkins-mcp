...
- **Server-side residuals closed (foundation):** HOST-003 Obtain→Jenkins AuthProvider when gateway provider Ready (Mode A); HOST-001 JWKS env wire on serve (`JENKINS_MCP_HTTP_JWKS_*`); HOST-011 mode matrix; **HOST-010 offline** Mode B `JWTRSBearerProvider` + JWT vault (Bearer Obtain; ID token rejected); GWY-002 `InboundClaimsFromJWTClaims` / `InboundClaimsFromRequestIdentity`; admin `GET /admin/v1/gateway/vault` (hash-only). Residual: mid-session subject rebind, **live Entra/jwt-auth-filter production pin (OAUTH-009)**, Mode C Live+Fetcher serve wire, GWY-003 live qualify.
