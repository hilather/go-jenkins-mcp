...
- Mutation ConfirmCooldown / MaxPreviews / TokenTTL operator resolve Done* (Waves 52–53); library Config negative still disables cooldown / unlimited previews for tests only; operator path cannot set 0/disable; serve fail-closes when ConfirmCooldown ≥ TokenTTL (`EnsureConfirmCooldownLessThanTokenTTL`) so cooldown cannot exhaust (or equal) the token window
...
- **Server-side residuals closed (foundation):** HOST-003 Obtain Ready path; HOST-001 JWKS + session fingerprint + process-local TTL JWKS refresh + **mid-session rebind residual offline expand** (PathPrefix + multi-user + lab JWT Alice/Bob + group claim fail-closed + order-stable + health exempt); multi-user Obtain opt-in (`JENKINS_MCP_GATEWAY_MULTI_USER`); HOST-010 Mode B vault; HOST-011 matrix; HOST-004/006 serve wire (SubjectLimiter + subject page tokens); GWY-001 Live opt-in; HOST-002/005 packaging. Residual: multi-instance JWKS HA, **live Entra/jwt-auth-filter (OAUTH-009) — offline expand is not live Entra Done**, real AgentCore pin, GWY-003 live SLOs, multi-user policy.Subject hot-swap.
