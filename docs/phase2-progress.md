...
- **Server-side free-lab Tier A Done\*:** HOST-003 Obtain Ready path; HOST-001 JWKS + session fingerprint + mid-session rebind offline expand; multi-user Obtain opt-in; HOST-010 Mode B vault; HOST-011 matrix; HOST-004/006 serve wire; GWY-001 Live opt-in; HOST-002/005 packaging; free labs (`live-jenkins-*`, `live-oauth-*`). **Site** Entra/jwt-auth-filter/AgentCore production pin = operator residual ([free-lab-qualification.md](gateway/free-lab-qualification.md)). **HOST-008 multi-pod HA cancelled** — scale via **multi-fleet** ([multi-fleet-rollout.md](fleet/multi-fleet-rollout.md)), not multi-replica gateway.
