...
- **Server-side residuals closed (foundation):** HOST-003 Obtain→Jenkins AuthProvider when gateway provider Ready (Mode A); HOST-001 JWKS env wire on serve (`JENKINS_MCP_HTTP_JWKS_*`); HOST-011 mode matrix + Mode B residual provider; admin `GET /admin/v1/gateway/vault` (hash-only). Residual: mid-session subject rebind, live Entra/jwt-auth-filter plugin, Mode C Live+Fetcher, GWY-003 live qualify.
- **HOST-002 / HOST-005 / HOST-007 / HOST-008 / GWY-004 packaging residual pass:** reverse-proxy deployment matrix + no-CORS-wildcard fail-closed; `/readyz` `gateway_ready` when `--gateway`; `deploy/gateway` non-root + limits + k8s probes; admin residual docs + health `enabledModes`; HA single-replica Tier A docs. OAUTH-009/010 offline vs mock-lab vs live residual matrices expanded (`jwt-auth-filter-qualification.md` §9, capability matrix §4). Live Entra / real jwt-auth-filter / AgentCore production remain residual.
