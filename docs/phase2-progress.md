...
- **Server-side residuals closed (foundation):** HOST-003 Obtain→Jenkins when Ready (A/B vault + C Live opt-in; whoAmI via Obtain); HOST-001 JWKS env + mid-session `Mcp-Session-Id` fingerprint fail-closed offline; HOST-010 Mode B JWT vault; HOST-011 matrix; HOST-004/006 isolation foundations; GWY-002 claim helpers; GWY-001 Live opt-in; admin vault hash-only + health `enabledModes`. **HOST-002/005/007/008/GWY-004 packaging:** reverse-proxy matrix + no-CORS-wildcard; `/readyz` `gateway_ready`; `deploy/gateway` non-root + limits + probes. Residual: multi-instance/under-load JWKS HA (process-local TTL refresh foundation landed), **live Entra/jwt-auth-filter (OAUTH-009)**, real AgentCore pin, GWY-003 live qualify, full list-tool subject wire.
