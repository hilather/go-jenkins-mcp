...
- **Server-side residuals closed (foundation):** HOST-003 Obtain→Jenkins AuthProvider when Ready (Mode A/B vault + Mode C Live opt-in; clear static; whoAmI via Obtain; ConsentRequired metadata; no other-subject fallthrough); HOST-001 JWKS env wire; HOST-010 offline Mode B JWT vault (Bearer; ID token rejected); HOST-011 mode matrix; GWY-002 claim helpers; admin vault hash-only status; **GWY-001 Live opt-in** (`JENKINS_MCP_GATEWAY_LIVE` + `HTTPTokenFetcher` — not full Done). Residual: mid-session rebind (HOST-001), **live Entra/jwt-auth-filter (OAUTH-009)**, real AgentCore pin, GWY-003 live qualify.
