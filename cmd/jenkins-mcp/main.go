...
under load; live Entra; live path-prefix origin pin matrix; prefer stdio for
pilot (ADR 0002).
...
func runServe(args []string) error {
	...
	if *httpAddr != "" {
		...
		log.Printf("http serve token policy: http_token_required=%v http_token_configured=%v http_subject_required=%v lab_identity=%v http_jwt_configured=%v http_jwt_required=%v max_body_bytes=%d path_prefix=%q gateway_ready_probe=%v expected_subject_pin=%v",
